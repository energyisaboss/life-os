...
import { Settings, PlusCircle, Trash2, Edit3, FolderPlus, FolderMinus, FilePlus, CheckCircle, Palette, XCircle } from 'lucide-react';
...
interface NewsWidgetSettingsProps {
  ...
  setCategories: React.Dispatch<React.SetStateAction<NewsCategory[]>>;
}

export function NewsWidget({ categories, setCategories }: NewsWidgetSettingsProps) {
  ...
  const handleSaveCategoryName = useCallback((categoryId: string) => {
    ...
    if (currentEditState.color && !isValidHexColor(currentEditState.color)) {
  ...
  const handleSaveFeed = useCallback(() => {
    ...
    setCategories(prev => prev.map(cat => {
      if (cat.id === editingFeed.categoryId) {
        ...
        return { ...cat, feeds: newFeeds, version: (cat.version || 0) + 1 };
      }
      return cat;
    }));
    ...
    setEditingFeed(null);
  }, [editingFeed, setCategories]);

  const handleDeleteFeed = useCallback((categoryId: string, feedId: string) => {
    setCategories(prev => prev.map(cat =>
      cat.id === categoryId ? { ...cat, feeds: cat.feeds.filter(f => f.id !== feedId), version: (cat.version || 0) + 1 } : cat
    ));
  ...
  const handleCategoryColorChange = useCallback((categoryId: string, newColor: string) => {
    setEditingCategoryState(prev => ({
        ...prev, 
        [categoryId]: { ...(prev[categoryId] || { name: '', color: getNextCategoryColor()}), color: newColor }
    }));
  }, []);
      ...
      <CardTitle className="text-lg font-semibold mb-4">News Settings</CardTitle>
        <CardContent className="p-1 space-y-4">
            ...
            <ScrollArea className="pr-1 custom-styled-scroll-area overflow-y-auto">
                ...
                {categories.map((category) => (
                    ...
                    <div className="flex justify-between items-center mb-2">
                        {category.isEditingName ? (
                            <div className="flex-grow flex flex-col space-y-2">
                                <Label htmlFor={`edit-category-name-${category.id}`} className="text-xs">Category Name</Label>
                                <Input 
                                ...
                                <div className="mb-3">
                                    ...
                                    <div className="flex flex-wrap items-center gap-1.5">
                                        ...
                                        <Input
                                            ...
                                            className={cn(
                                                "h-7 w-20 text-xs",
                                                editingCategoryState[category.id]?.color && !isValidHexColor(editingCategoryState[category.id]?.color || '') && editingCategoryState[category.id]?.color !== '' ? "border-destructive focus-visible:ring-destructive" : ""
                                            )}
                                            maxLength={7}
                                        />
                                    </div>
                                ...
                                <div className="flex justify-end gap-2">
                                    <Button variant="outline" size="sm" onClick={() => handleCancelEditCategoryName(category.id)}>Cancel</Button>
                                    <Button size="sm" onClick={() => handleSaveCategoryName(category.id)} disabled={!editingCategoryState[category.id]?.name?.trim() || (editingCategoryState[category.id]?.color && !isValidHexColor(editingCategoryState[category.id]?.color))}>Save</Button>
                                </div>
                            </div>
                        ) : (
                    ...
                    <div className="pl-2 border-l-2 border-border/50 space-y-2 mb-2">
                        {category.feeds.map(feed => (
                          editingFeed?.categoryId === category.id && editingFeed?.feedId === feed.id ? (
                            <div key={feed.id} className="text-xs p-1.5 rounded bg-background/50 space-y-2">
                                <Label htmlFor={`edit-feed-label-${feed.id}`} className="text-xs">Feed Label (Optional)</Label>
                                <Input
                                    id={`edit-feed-label-${feed.id}`}
                                    type="text"
                                ...
                                />
                                <Label htmlFor={`edit-feed-url-${feed.id}`} className="text-xs">Feed URL*</Label>
                                <Input
                                ...
                                <div className="flex justify-end gap-2 mt-3">
                                    ...
                                    <Button size="sm" onClick={handleSaveFeed} disabled={!editingFeed?.url.trim() || !editingFeed?.url.toLowerCase().startsWith('http')}>Save Feed</Button>
                                </div>
                            </div>
                          ) : (
                        ...
                        {/* Add New Feed Form */}
                        {editingFeed?.categoryId === category.id && editingFeed?.feedId === undefined && ( // Check if adding a new feed to THIS category
                             <div className="text-xs p-1.5 rounded bg-background/50 space-y-2">
                                <Label htmlFor={`new-feed-label-${category.id}`} className="text-xs">Feed Label (Optional)</Label>
                                <Input
                                    id={`new-feed-label-${category.id}`}
                                    type="text"
                                    placeholder="e.g., TechCrunch News"
                                    value={editingFeed.userLabel}
                                    onChange={(e) => setEditingFeed(ef => ef ? { ...ef, userLabel: e.target.value } : null)}
                                    className="h-8 text-xs"
                                />
                                <Label htmlFor={`new-feed-url-${category.id}`} className="text-xs">Feed URL*</Label>
                                <Input
                                    id={`new-feed-url-${category.id}`}
                                    type="url"
                                    placeholder="https://example.com/feed.xml"
                                    value={editingFeed.url}
                                    onChange={(e) => setEditingFeed(ef => ef ? { ...ef, url: e.target.value } : null)}
                                ...
                                <div className="flex justify-end gap-2 mt-3">
                                    <Button variant="outline" size="sm" onClick={() => setEditingFeed(null)}>Cancel</Button>
                                    <Button size="sm" onClick={handleSaveFeed} disabled={!editingFeed?.url.trim() || !editingFeed?.url.toLowerCase().startsWith('http')}>Add Feed</Button>
                                </div>
                            </div>
                        )}


                        {category.feeds.length === 0 && !editingFeed?.categoryId === category.id && <p className="text-xs text-muted-foreground pl-1.5">No feeds in this category.</p>}
                    </div>
                    {!editingFeed || editingFeed.categoryId !== category.id ? (
                        <Button 
                            variant="outline" 
                            size="sm" 
                            className="w-full h-8 text-xs mt-1" 
                            onClick={() => handleStartAddOrEditFeed(category.id)}
                            disabled={category.feeds.length >= MAX_FEEDS_PER_CATEGORY}
                        >
                ...
                {categories.length === 0 && <p className="text-xs text-muted-foreground text-center py-3">No categories created yet. Add one above.
